      ...
      try {
        ...
        if (createError) {
          ...
          if (createError.message?.includes("already been registered")) {
            console.log(`User ${email} exists in auth but not in profiles, fetching and linking...`);
            ...
            if (existingAuthUser) {
              // Upsert profile to link to company
              await supabaseAdmin
                .from("profiles")
                .upsert({
                      ...
                      html: `
                        ...
                        <div style="max-width: 560px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden;">
                          <div style="background: linear-gradient(135deg, #8B5CF6, #6366F1); padding: 32px; text-align: center;">
                            <h1 style="color: white; margin: 0; font-size: 28px;">FinCare</h1>
                          </div>
                          <div style="padding: 32px;">
                            <h2 style="color: #18181b;">${displayName},</h2>
                            <p style="color: #52525b;">Votre compte FinCare est rattaché à <strong>${company.name}</strong>.</p>
                            <p style="color: #52525b;">Cliquez ci-dessous pour définir votre mot de passe :</p>
                            <div style="text-align: center; margin: 32px 0;">
                              <a href="${linkData.properties.action_link}" style="display: inline-block; background: linear-gradient(135deg, #8B5CF6, #6366F1); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600;">Créer mon mot de passe</a>
                            </div>
                          </div>
                        </div></body></html>`,
                    });
              ...
              results.push({ email, success: true, userId: existingAuthUser.id, emailSent });
              console.log(`Linked existing auth user ${email} to company ${companyId}`);
              continue;
            }
          }

...
import { AtlasResultsStepper } from "./ocr/AtlasResultsStepper";
  ...
  const saveAnalysis = useCallback(async (analysisData: AvisData) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("ocr_avis_imposition_analyses" as any).insert({
        ...
        nombre_parts: n(analysisData.contribuable?.nombre_parts),
        // Années
        annee_revenus: n(analysisData.annees?.annee_revenus),
        annee_imposition: n(analysisData.annees?.annee_imposition),
        // Revenus
        salaires_traitements_bruts: n(analysisData.revenus?.salaires_traitements_bruts),
        abattement_10_pct: n(analysisData.revenus?.abattement_10_pct),
        salaires_nets_imposables: n(analysisData.revenus?.salaires_nets_imposables),
        revenus_fonciers_nets: n(analysisData.revenus?.revenus_fonciers_nets),
        revenus_capitaux_mobiliers: n(analysisData.revenus?.revenus_capitaux_mobiliers),
        plus_values_mobilieres: n(analysisData.revenus?.plus_values_mobilières),
        bic_bnc_ba: n(analysisData.revenus?.bic_bnc_ba),
        pensions_retraites: n(analysisData.revenus?.pensions_retraites),
        autres_revenus: n(analysisData.revenus?.autres_revenus),
        revenu_brut_global: n(analysisData.revenus?.revenu_brut_global),
        charges_deductibles: n(analysisData.revenus?.charges_deductibles),
        revenu_net_global: n(analysisData.revenus?.revenu_net_global),
        abattements_speciaux: n(analysisData.revenus?.abattements_speciaux),
        revenu_net_imposable: n(analysisData.revenus?.revenu_net_imposable),
        revenu_fiscal_reference: n(analysisData.revenus?.revenu_fiscal_reference),
        // Impôt
        impot_brut_progressif: n(analysisData.impot?.impot_brut_progressif),
        taux_marginal_imposition_pct: n(analysisData.impot?.taux_marginal_imposition_pct),
        taux_moyen_pct: n(analysisData.impot?.taux_moyen_imposition_pct),
        plafonnement_quotient_familial: n(analysisData.impot?.plafonnement_quotient_familial),
        reductions_impot: n(analysisData.impot?.reductions_impot),
        credits_impot: n(analysisData.impot?.credits_impot),
        impot_net_avant_contributions: n(analysisData.impot?.impot_net_avant_contributions),
        prelevement_forfaitaire_unique: n(analysisData.impot?.prelevement_forfaitaire_unique),
        contributions_sociales_revenus_capital: n(analysisData.impot?.contributions_sociales_revenus_capital),
        taxe_habitation: n(analysisData.impot?.taxe_habitation),
        impot_net_total: n(analysisData.impot?.impot_net_total),
        total_a_payer: n(analysisData.impot?.total_a_payer),
        mensualisation_ou_prelevement: n(analysisData.impot?.mensualisation_ou_prelevement),
        // Prélèvement à la source
        taux_pas_pct: n(analysisData.prelevement_source?.taux_pas_pct),
        montant_preleve_annee_n: n(analysisData.prelevement_source?.montant_preleve_annee_n),
        solde: n(analysisData.prelevement_source?.solde_a_payer_ou_rembourser),
        // Meta
        ...
        // PER
        plafond_per_declarant_1: n(analysisData.plafonds_per?.plafond_declarant_1),
        plafond_per_declarant_2: n(analysisData.plafonds_per?.plafond_declarant_2),
